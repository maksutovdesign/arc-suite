...
export async function getPilotSummary(): Promise<PilotSummary> {
  ...
  return {
    ...
    endpoints: [
      ...
      { method: "GET", path: "/api/escrow/overview", description: "Agent deals, milestones, disputes and contract events" },
      { method: "POST", path: "/api/escrow/deals", description: "Create an idempotent milestone escrow agreement" },
      { method: "POST", path: "/api/escrow/actions", description: "Submit, dispute, release or refund an escrow milestone" },
...
function normalizeOpsHealthCheck(input: unknown): OpsHealthCheckInput | null {
  ...
  return {
    ...
    monitorName: normalizeOptionalText(data.monitorName, 120) ?? "Kestrel Production Monitor",
    results,
