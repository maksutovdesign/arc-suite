...
function mapApiListing(api: BackendApiListing): ApiListing {
  ...
  return {
    ...
    description: seed?.description ?? `${api.name} is an x402 API listed in the Kestrel Marketplace live catalog.`,
    longDescription: seed?.longDescription ?? `${api.name} is served from the Kestrel backend catalog with live pricing, uptime, request volume, and reputation requirements. Agents can request API access using their current Treasury budget and Reputation score.`,
    category,
