...
import { AGENTS, TIER_CONFIG, SCORE_HISTORY, type Agent } from "@/data/mock"
...
export function ReputationAgentsGrid({ agents, source }: Props) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <PageHeader
        title="All Agents"
        subtitle={`${agents.length} registered · ${agents.filter(a => a.verified).length} verified · ${agents.filter(a => a.tier !== "new").length} established · ${sourceLabel}`}
        icon={ShieldCheck}
        glow
