...
import { CheckCircle, Activity, TrendingUp, TrendingDown, Clock, ArrowLeft, Zap, AlertTriangle, Globe, Fingerprint, ShieldCheck } from "lucide-react"
...
export default async function AgentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const passportId = `erc8004:${agent.network.toLowerCase()}:${agent.id}`
  const accessReady = agent.verified && agent.score >= 700 && agent.successRate >= 97
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
          ...
          {/* Agent Passport */}
          <div className="rounded-2xl p-5"
            style={{ background: "linear-gradient(160deg,#263a52,#1e3247)", border: "1px solid rgba(95,191,255,0.14)" }}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <Fingerprint className="size-4" style={{ color: "#5FBFFF" }} />
                  <p className="text-sm font-semibold text-white">Agent Passport</p>
                </div>
                <p className="mt-1 text-[11px] font-mono" style={{ color: "#5FBFFF" }}>{passportId}</p>
              </div>
              <span
                className="rounded-full px-2.5 py-1 text-[10px] font-semibold"
                style={{
                  background: accessReady ? "rgba(52,211,153,0.1)" : "rgba(245,158,11,0.1)",
                  border: `1px solid ${accessReady ? "rgba(52,211,153,0.25)" : "rgba(245,158,11,0.25)"}`,
                  color: accessReady ? "#34d399" : "#f59e0b",
                }}
              >
                {accessReady ? "Access ready" : "Needs review"}
              </span>
            </div>

            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                { label: "Identity", value: agent.verified ? "Verified" : "Unverified" },
                { label: "Owner", value: agent.owner },
                { label: "Settlement", value: "x402 + USDC" },
                { label: "Controls", value: "Limits + policy" },
              ].map((item) => (
            ...
            <div className="mt-3 flex items-center gap-2 text-[11px]" style={{ color: "#94a3b8" }}>
              <ShieldCheck className="size-3.5 shrink-0" style={{ color: "#34d399" }} />
              <span>Portable identity, reputation, payment history, and policy signals for agentic financial workflows.</span>
            </div>
          </div>
